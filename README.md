...
```bash
# 1) 纯逻辑自检：检索打分、双过期、裁剪、上传与导入分析（无依赖，111 项）
python tests/selftest.py

# 2) 集成冒烟：在真实 AstrBot 源码环境里验证注册契约、Web API 与宿主模式（164 项）
...
真的跑一遍 `_decide()` 和全部 13 个 Page 接口，并校验 Page 目录与 i18n 文件齐备。
其中三处是端到端的：

* **宿主模式**：造一个真实的常规插件（`Star` 子类 + `@filter.llm_tool` + `@filter.command`）
  走一遍导入，断言实例被创建、工具与命令都被绑定到该实例、**并且真的调用成功**
...
| AstrBot | 核对方式 | 结果 |
|---|---|---|
| **4.26.7** | 逐行读源码 ＋ `smoke_astrbot.py` **真机跑通**（真实导入 AstrBot、真实构造 `ToolSet`/`FunctionTool`、真实调用注入钩子与全部 13 个 Page 接口，含宿主模式端到端），275 项断言全绿 | 全部成立 |
